...
#############################################
# IMPA CRM - Instalador Automatico
...
echo "╔═══════════════════════════════════════════════════════════╗"
echo "║                                                           ║"
echo "║     IMPA CRM - Instalador Automatico                       ║"
echo "║                      v2.1.11                              ║"
echo "║                                                           ║"
echo "╚═══════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
# Database
DATABASE_URL="postgresql://crm_impa:${DB_PASSWORD}@localhost:5432/crm_impa_db?schema=public"
...
docker stop crm_impa_postgres 2>/dev/null || true
docker rm crm_impa_postgres 2>/dev/null || true

docker run -d \
  --name crm_impa_postgres \
  --restart always \
  -e POSTGRES_USER=crm_impa \
  -e POSTGRES_PASSWORD=${DB_PASSWORD} \
  -e POSTGRES_DB=crm_impa_db \
  -p 5432:5432 \
  -v crm_impa_postgres_data:/var/lib/postgresql/data \
  postgres:15-alpine
...
docker stop crm_impa_redis 2>/dev/null || true
docker rm crm_impa_redis 2>/dev/null || true
...
for i in {1..30}; do
  if docker exec crm_impa_postgres pg_isready -U crm_impa > /dev/null 2>&1; then
...
pm2 delete crm-impa-backend 2>/dev/null || true
pm2 delete crm-impa-frontend 2>/dev/null || true
...
cd $APP_DIR/backend
pm2 start dist/server.js --name crm-impa-backend
log_success "Backend iniciado na porta ${PORT}"

# Iniciar frontend com PM2
log_info "Iniciando frontend com PM2..."
cd $APP_DIR/frontend
pm2 start "serve -s dist -l ${FRONTEND_PORT}" --name crm-impa-frontend
...
echo -e "Usuario:            crm_impa"
echo -e "Senha:              ${DB_PASSWORD}"
echo -e "Database:           crm_impa_db"
echo ""
echo -e "${YELLOW}=== Comandos Uteis ===${NC}"
echo ""
echo -e "Ver logs backend:   ${BLUE}pm2 logs crm-impa-backend${NC}"
echo -e "Ver logs frontend:  ${BLUE}pm2 logs crm-impa-frontend${NC}"
