...
const flowNodeSchema = z.object({
  ...
  type: z.enum([
    ...
    'MENU', 'BUTTONS', 'CAROUSEL', 'LIST', 'CONDITION', 'DELAY', 'SET_VARIABLE',
    'HTTP_REQUEST', 'TRANSFER', 'GO_TO_FLOW', 'END',
    'LLM', 'AI_AGENT', 'KNOWLEDGE_RETRIEVAL', 'CODE', 'TEMPLATE',
    'LOOP', 'PARALLEL', 'QUESTION_CLASSIFIER', 'ITERATION', 'HUMAN_INPUT', 'APPROVAL',
    'SEND_MESSAGE', 'UPDATE_CONTACT', 'ASSIGN_CONVERSATION', 'ADD_TAG', 'MOVE_CARD',
...
export async function flowRoutes(fastify: FastifyInstance) {
  ...
  // List all flows
  fastify.get('/', { preHandler: [requirePermission('flows:read')] }, async (request, reply) => {
  ...
  fastify.get('/:id', { preHandler: [requirePermission('flows:read')] }, async (request, reply) => {
    const companyId = getCompanyId(request)
  ...
  // Create new flow
  fastify.post('/', { preHandler: [requirePermission('flows:manage'), enforcePlanLimit('maxFlows')] }, async (request, reply) => {
    try {
      const companyId = getCompanyId(request)
      ...
      return reply.status(500).send({ error: 'Erro ao criar fluxo' })
    }
  })

  // Update flow metadata
  fastify.put('/:id', { preHandler: [requirePermission('flows:manage')] }, async (request, reply) => {
    const companyId = getCompanyId(request)
    const { id } = request.params as { id: string }
  ...
  fastify.put('/:id/canvas', { preHandler: [requirePermission('flows:manage')] }, async (request, reply) => {
    const companyId = getCompanyId(request)
    const { id } = request.params as { id: string }
  ...
  // Delete flow
  fastify.delete('/:id', { preHandler: [requirePermission('flows:manage')] }, async (request, reply) => {
    const companyId = getCompanyId(request)
    const { id } = request.params as { id: string }
  ...
  // Duplicate flow
  fastify.post('/:id/duplicate', { preHandler: [requirePermission('flows:manage')] }, async (request, reply) => {
    const companyId = getCompanyId(request)
    const { id } = request.params as { id: string }
  ...
  // Get flow statistics
  fastify.get('/:id/stats', { preHandler: [requirePermission('flows:read')] }, async (request, reply) => {
    const companyId = getCompanyId(request)
    const { id } = request.params as { id: string }
  ...
  fastify.get('/:id/sessions', { preHandler: [requirePermission('flows:read')] }, async (request, reply) => {
    const companyId = getCompanyId(request)
    const { id } = request.params as { id: string }
  ...
  fastify.post('/test-http', { preHandler: [requirePermission('flows:manage')] }, async (request, reply) => {
    ...
    try {
      ...
      return reply.status(200).send({ success: false, error: 'Erro na requisi��o' })
    }
  })

  // Stop an active execution without deleting its history
  fastify.post('/:id/sessions/:sessionId/stop', { preHandler: [requirePermission('flows:manage')] }, async (request, reply) => {
    const companyId = getCompanyId(request)
    ...
    const now = new Date()
    const stopped = await prisma.flowSession.updateMany({
      where: {
        id: sessionId,
        flowId: id,
        isActive: true,
    ...
    if (stopped.count === 0) {
      ...
      return reply.status(409).send({ error: 'Execution is already stopped' })
    }

    return { success: true }
  })

  // Delete a finalized execution from the flow history
  fastify.delete('/:id/sessions/:sessionId', { preHandler: [requirePermission('flows:manage')] }, async (request, reply) => {
    const companyId = getCompanyId(request)
    const { id, sessionId } = request.params as { id: string; sessionId: string }
    ...
    if (session.isActive) {
      return reply.status(409).send({ error: 'Active executions cannot be deleted' })
    }
  ...
  // Listar sessões aguardando HUMAN_INPUT/APPROVAL para a empresa logada
  fastify.get('/sessions/awaiting-input', { preHandler: [requirePermission('flows:read')] }, async (request) => {
    const companyId = getCompanyId(request)
    const sessions = await prisma.flowSession.findMany({
      where: { isActive: true, waitingInput: true, flow: { companyId } },
  ...
  // Resume HUMAN_INPUT/APPROVAL session
  fastify.post('/sessions/:sessionId/human-input', { preHandler: [requirePermission('flows:manage')] }, async (request, reply) => {
    const companyId = getCompanyId(request)
    const user = request.user as any
    const { sessionId } = request.params as { sessionId: string }
    const body = request.body as { value?: any; approved?: boolean }

    // Verificar dono
    const session = await prisma.flowSession.findUnique({
      where: { id: sessionId },
      include: { flow: { select: { companyId: true } } },
    })
    if (!session || session.flow.companyId !== companyId) {
      return reply.status(404).send({ error: 'Sessão não encontrada' })
    }

    const engine = getFlowEngine()
    if (!engine) return reply.status(500).send({ error: 'FlowEngine não inicializado' })

    const result = await engine.resumeWithHumanInput(sessionId, {
      value: body.value,
      approved: body.approved,
      reviewedByUserId: user?.id || user?.userId,
