...
export async function automationRoutes(fastify: FastifyInstance) {
  ...
  fastify.register(async (protectedRoutes) => {
    ...
    protectedRoutes.get(
      '/',
      { preHandler: [requirePermission('automations:read')] },
    ...
    protectedRoutes.post(
      '/',
      { preHandler: [requirePermission('automations:manage')] },
      async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    // Update automation
    protectedRoutes.put<{ Params: { id: string } }>(
      '/:id',
      { preHandler: [requirePermission('automations:manage')] },
      async (request, reply) => {
        const { id } = request.params
    ...
    protectedRoutes.patch<{ Params: { id: string } }>(
      '/:id/toggle',
      { preHandler: [requirePermission('automations:manage')] },
      async (request, reply) => {
        const { id } = request.params
    ...
    // Get automation logs
    protectedRoutes.get<{ Params: { id: string }; Querystring: { page?: string; limit?: string; status?: string } }>(
      '/:id/logs',
      { preHandler: [requirePermission('automations:read')] },
      async (request, reply) => {
        const { id } = request.params
