...
async function getCloudApiCredentials(companyId: string) {
  ...
  // Decrypt accessToken from DB
  const accessToken = decryptSafe(instance.accessToken) as string
  ...
  if (!appId) {
    appId = await fetchAppIdFromToken(accessToken)
...
export async function templateRoutes(fastify: FastifyInstance) {
  ...
  // List templates
  fastify.get('/', { preHandler: [requirePermission('templates:read')] }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  fastify.post('/sync', { preHandler: [requirePermission('templates:manage')] }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  fastify.post('/', { preHandler: [requirePermission('templates:manage'), enforcePlanLimit('maxTemplates')] }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  // Delete template
  fastify.delete<{ Params: { id: string } }>('/:id', { preHandler: [requirePermission('templates:manage')] }, async (request, reply) => {
